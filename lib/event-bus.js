...
class BossClaudeEventBus extends EventEmitter {
  ...
  async initializeRedis() {
    try {
      ...
      if (!redis) {
        console.error('[event-bus] Redis not available - using local-only events');
        return;
      }
      ...
      if (!subscriber) {
        console.error('[event-bus] Redis subscriber not available');
        return;
      }
      ...
      this.redisSubscription = subscribeToChannel('boss:events', (message) => {
        try {
          ...
          console.error(`[event-bus] Received Redis event: ${event.name} from ${event.origin}`);
        } catch (err) {
      ...
      console.error('[event-bus] Redis pub/sub initialized successfully');
    } catch (err) {
  ...
  async publishToRedis(event) {
    ...
    try {
      ...
      console.error(`[event-bus] Published to Redis: ${event.name}`);
    } catch (err) {
